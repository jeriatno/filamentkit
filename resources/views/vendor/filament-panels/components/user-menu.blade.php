...
<x-filament::dropdown placement="bottom-end" teleport :attributes="\Filament\Support\prepare_inherited_attributes($attributes)->class(['fi-user-menu'])">
  <x-slot name="trigger">
    <button aria-label="{{ __('filament-panels::layout.actions.open_user_menu.label') }}" type="button" class="shrink-0">
  ...
  <x-filament::dropdown.list>
    ...
    <x-filament::dropdown.list.item color="danger" icon="heroicon-o-arrow-left-on-rectangle"
      @click="$dispatch('open-modal', { id: 'logout-modal' })" class="cursor-pointer">
      {{ __('Logout') }}
    </x-filament::dropdown.list.item>
